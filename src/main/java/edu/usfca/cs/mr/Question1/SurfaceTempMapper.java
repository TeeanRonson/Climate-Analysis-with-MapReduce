...
public class SurfaceTempMapper
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        FloatWritable surfaceTemp = new FloatWritable();

        //Set values
        String location = Geohash.encode(grab.get_Latitude(), grab.get_Longitude(), 3);
        dl.set(location, grab.getUTCDateByMonth());
        surfaceTemp.set(grab.get_Surface_Temp());

        System.out.println(dl.toString() + " " + surfaceTemp);
        context.write(dl, surfaceTemp);
